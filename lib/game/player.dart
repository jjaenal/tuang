...
class Player extends SpriteComponent with HasGameReference<MyGame> {
  ...
  Future<void> onLoad() async {
    ...
    try {
      ...
      LoggingService.log('player_sprite_load_failed', fields: {'error': e.toString()});
    }
  }
  ...
  void update(double dt) {
    ...
    if (sprite == null) {
      paint.color = game.playerSpeedMultiplier > 1.0 ? _boostColor : _defaultColor;
    } else {
      // Jika ada sprite, bisa mengubah opacity atau skala untuk efek boost
      opacity = game.playerSpeedMultiplier > 1.0 ? 0.8 : 1.0;
      scale = game.playerSpeedMultiplier > 1.0 ? Vector2.all(1.1) : Vector2.all(1.0);
    }
